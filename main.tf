locals {
  ...
  defaults = {
    label_order         = ["project", "environment", "family", "application", "attributes"]
  ...
  input = {
    ...
    family      = var.family == null ? var.context.family : var.family
  ...
  project         = lower(replace(coalesce(local.input.project, local.sentinel), local.regex_replace_chars, local.replacement))
  environment     = lower(replace(coalesce(local.input.environment, local.sentinel), local.regex_replace_chars, local.replacement))
  family          = lower(replace(coalesce(local.input.family, local.sentinel), local.regex_replace_chars, local.replacement))
